...
export class TemperatureComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.dataSubscription = this.dataService.getTemperatureData().subscribe((response: any[]) => {
      this.inProgress = true;

      this.data = response;
      this.chartService.toStampFormat(this.data);
      this.chartService.renderChart(this.data, 'Температура');
  ...
  tempDateChange() {
    ...
    this.chartService.renderChart(newData, 'Температура');
  }
